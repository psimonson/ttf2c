...
#define PITCH		27	/* bytes per glyph */
 ...
 */
static void out_xbm(unsigned char **image, const char *name,
...
int main(int argc, char **argv)
{
	...
	for(i = 0; i < nglyphs; i++) {
		image[i] = malloc(sizeof(unsigned char)*PITCH);
		if(image[i] != NULL)
			memset(image[i], 0, sizeof(unsigned char)*PITCH);
	}
	pen_x = 100;
	pen_y = 100;
	for(g = 0; g < nglyphs; g++) {
